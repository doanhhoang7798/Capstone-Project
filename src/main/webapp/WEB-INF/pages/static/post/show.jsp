...
<!--                 /// -->
	...
	<script type="text/javascript">
			  ...
			  if (report_mgs == "") {
			      ...
			      $(".report-dialog").css({ margin: "166px", "position": "fixed", display: "block", "z-index": " 999" });
			  }
			  ...
			  $(".comment_edit").click(function () {
				...
				if(afterId == id){
				    ...
				    $(".show_content_" + afterId).css("display", "block");
				    $(".open-update_" + id).css("display", "block");
				    $(".show_content_" + id).css("display", "none");
				    $('#afterId').val(id);
				}
			     ...
			     $("#c-e-" + id).click(function (evt) {
			    	 console.log("CLICK SEND DATA", id)
			        evt.preventDefault();
			        var fileNull = new File([""], "filename");
			        var file = $("#c-e-image-" + id).prop("files")[0] == undefined ? fileNull : $("#c-e-image-" + id).prop("files")[0];
			        var content = $("#c-e-content-" + id).val();
			        ...
			        $("#c-e-confirm").click(function () {
			        	console.log("CLICK CONFIRM CHANGE")
			           $.ajax({
			              type: "POST",
			  ...
			  $(".comment_delete").click(function () {
			     ...
			     $("#c-d-confirm").click(function () {
			        $.ajax({
			           ...
			           contentType: false,
			           processData: false,
			           url: "${pageContext.request.contextPath}/comment/delete/" + id,
			           success: function (data) {
			              $("#c-count").html("( " + data + " )");
			              $("#refresh").load(location.href + " #refresh");
			           },
			  ...
			  $("#comment-create").click(function (evt) {
				  ...
				  if ($("#c-c-image").prop("files")[0] == null && $('#c-c-content').text() == "") {
					  document.getElementById('c-c-content').setCustomValidity("This field cannot be left blank");
				  } else {
					  console.log("PASSS VALIDATE")
					     var fileNull = new File([""], "filename");
					     var file = $("#c-c-image").prop("files")[0] == undefined ? fileNull : $("#c-c-image").prop("files")[0];
